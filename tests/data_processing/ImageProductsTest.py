...
import numpy as np
import cv2
from src.data_processing.ImageProducts import (ncc, get_image_product, image_product_pow, image_product_exp_repeated,
...
class TestNCC(unittest.TestCase):
    ...
    def test_get_power_image_products(self):
        ...
        self.assertEqual(result2(mainImg, tempImg), image_product_pow(ncc, 2)(mainImg, tempImg))

        result3 = get_image_product("ncc_pow_2.")
        self.assertIsInstance(result3, Callable)
        self.assertEqual(result3(mainImg, tempImg), image_product_pow(ncc, 2)(mainImg, tempImg))

        result4 = get_image_product("ncc_pow_1.5")
        self.assertIsInstance(result4, Callable)
        self.assertEqual(result4(mainImg, tempImg), image_product_pow(ncc, 1.5)(mainImg, tempImg))
