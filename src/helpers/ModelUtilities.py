...
class SimpleCNN2_aps_dropout(nn.Module):
...
class SimpleCNN2_dropout(nn.Module):
...
class SimpleCNN2_CBAM(nn.Module):
    def __init__(self, dimensions=128, padding_mode='circular', dropout_rate=0.3):
        super().__init__()
...
class SimpleCNN2_CBAM_dropout(nn.Module):
    def __init__(self, dimensions=128, padding_mode='circular', dropout_rate=0.3):
        super().__init__()
...
class SimpleCNN4_aps_CBAM(nn.Module):
    def __init__(self, dimensions=10, padding_mode='circular', dropout_rate=0.3):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 16, 3, padding=1, padding_mode=padding_mode)
        self.bn1 = nn.BatchNorm2d(16)
...
class SimpleCNN6_aps_CBAM(nn.Module):
    def __init__(self, dimensions=10, padding_mode='circular', dropout_rate=0.3):
        super().__init__()
        self.conv1 = nn.Conv2d(1, 16, 3, padding=1, padding_mode=padding_mode)
        self.bn1 = nn.BatchNorm2d(16)
...
class SimpleCNN4_CBAM_2fc(nn.Module):
    def __init__(self, dimensions=128, padding_mode='circular', dropout_rate=0.3):
        super().__init__()
...
class SimpleCNN6_2fc(nn.Module):
    def __init__(self, dimensions=128, padding_mode='circular'):
        super().__init__()
...
class SimpleCNN6_CBAM_2fc(nn.Module):
    def __init__(self, dimensions=128, padding_mode='circular', dropout_rate=0.3):
        super().__init__()
...
class AddCoordinates(nn.Module):
    ...
    def forward(self, x):
        ...
        y_coords = torch.linspace(-1, 1, height, device=x.device).repeat(1, width, 1).transpose(1, 2)
        x_coords = torch.linspace(-1, 1, width, device=x.device).repeat(1, height, 1)
        coords = torch.stack([x_coords, y_coords], dim=1).unsqueeze(0).repeat(batch_size, 1, 1, 1)
        
        if self.with_r:
            rr = torch.sqrt(torch.pow(x_coords, 2) + torch.pow(y_coords, 2)).unsqueeze(0).unsqueeze(0)
            rr = rr / torch.max(rr)
            rr = rr.repeat(batch_size, 1, 1, 1)
...
class CoordConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        super().__init__()
        self.addcoords = AddCoordinates(with_r=True)
...
class SimpleCNN4_CBAM_Coord(nn.Module):
    def __init__(self, dimensions=128, padding_mode='circular', dropout_rate=0.3):
        super().__init__()

        self.conv1 = CoordConvBlock(1, 16) 
