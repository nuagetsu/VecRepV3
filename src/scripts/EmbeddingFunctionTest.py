...
for i in pencorr_indices:
    ...
    # Plot Matrix A -- plot abit wrong for matrix A
    plt.figure(figsize=(7, 5))
    plt.imshow(np.flip(matrixA, axis=0), cmap='viridis', aspect='auto')
    plt.title(f"Matrix A using BFMethod - {embeddingType}", fontsize=16)
    plt.colorbar(label="Value")
    plt.xlabel("Columns of Matrix A")
    plt.ylabel("Rows of Matrix A")

    plt.xticks(ticks=np.arange(np.flip(matrixA, axis=0).shape[1]), labels=np.arange(np.flip(matrixA, axis=0).shape[1]))
    plt.yticks(ticks=np.arange(np.flip(matrixA, axis=0).shape[0]), labels=np.arange(np.flip(matrixA, axis=0).shape[0] - 1, -1, -1))
    plt.gca().invert_yaxis() 

    for (row, col), val in np.ndenumerate(np.flip(matrixA, axis=0)):
        color = 'white' if val < 0.5 * (np.flip(matrixA, axis=0).max() - np.flip(matrixA, axis=0).min()) + np.flip(matrixA, axis=0).min() else 'black'
        plt.text(col, row, f"{val:.2f}", ha='center', va='center', fontsize=8, color=color)
